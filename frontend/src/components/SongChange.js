            ...
            <ul className="list-group">
                {songChanges.map((songChange, index) => (
                    <li key={index} className="list-group-item">
                        ...
                        <p>Change Occurred At: {new Date(songChange.changeOccurredAt).toLocaleString()}</p>
                        <p>Duration: {Math.floor(songChange.durationMs/1000/60)}:{Math.floor(songChange.durationMs/1000%60)}min </p>
                        <p>Spotify URL: <a href={songChange.spotifyExternalUrl.match(/spotify=(https:\/\/open.spotify.com\/track\/[^\}]*)/)[1]} target="_blank" rel="noopener noreferrer">Listen</a></p>
